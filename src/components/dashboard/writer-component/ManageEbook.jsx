...
const ManageEbook = ({writerEbooksData,user}) => {
  ...
  // ১. পাবলিশ/আনপাবলিশ টগল হ্যান্ডলার (MongoDB _id এবং বর্তমান status রিসিভ করবে)
  const handleTogglePublish = (id, currentStatus) => {
    const nextStatus = currentStatus === "published" ? "unpublished" : "published";
    console.log(`Updating status for book ID: ${id} to ${nextStatus}`);
    // এখানে তোমার backend API-তে PATCH/PUT রিকোয়েস্ট পাঠাবে
    // axios.patch(`/api/ebooks/${id}`, { status: nextStatus })
  };
      ...
      <div className="">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-2 text-sm">
            Overview and management of your published manuscripts. Review sales, adjust pricing, and control visibility.
          </p>
        </div>
...
{writerEbooksData.length===0? (
          <div className="bg-white rounded-2xl border border-gray-200/80 shadow-[0_2px_12px_-3px_rgba(0,0,0,0.04)] p-12 sm:p-20 flex flex-col items-center text-center">
            ...
            {/* টেক্সট কন্টেন্ট */}
            <h3 className="text-xl font-bold text-[#111827] mb-2">No Ebooks Found</h3>
            <p className="text-gray-500 text-sm max-w-sm mb-8 leading-relaxed">
              You haven&apos;t written or uploaded any ebooks yet. Prepare your next masterpiece and share it with the Fable library!
            </p>
          ...
          <Table className='bg-white p-0 rounded-2xl  '>
            <Table.ScrollContainer className=''>
              <Table.Content className='  ' aria-label="Writer's ebooks management table">
                <Table.Header className=' rounded-none bg-(--color-primary)/10 ' >
                  <Table.Column isRowHeader className="text-xs font-bold text-gray-500 tracking-wider uppercase bg-slate-50/70 p-4">
                    Book Title
                  </Table.Column>
                ...
                <Table.Body className='' >
                  {writerEbooksData.map((ebook) => {
                    ...
                    return (
                      <Table.Row key={ebook._id} className="border-b border-(--color-border) hover:bg-slate-50/50 transition-colors">
                        ...
                        <Table.Cell className="p-4 font-medium text-gray-700 text-sm sm:text-base">
                          {numericPrice === 0 ? "$0.00" : `$${numericPrice.toFixed(2)}`}
                        </Table.Cell>
                        ...
                        <Table.Cell className="p-4">
                          ...
                          >
                            <span className={`w-1.5 h-1.5 rounded-full ${isPublished ? "bg-emerald-500" : "bg-[#6D28D9]"}`} />
                            {ebook.status}
                          </span>
                        </Table.Cell>
                          ...
                          <div className="flex items-center justify-end gap-4 sm:gap-6 pr-4">
                            ...
                            <label className="relative inline-flex items-center cursor-pointer select-none">
                              <input
                                type="checkbox"
                                checked={isPublished}
                                onChange={() => handleTogglePublish(ebook._id, ebook.status)}
